class Utils {
  ...
  static _extractAlphaNumericToken(pointer, index) {
    let token = "";
    this._iterateThroughNonTokens(pointer, index)
    while (index[0] < pointer.length && Utils._isAlphaNumericTokenCode(pointer.charCodeAt(index[0]))) {
